...
const TeamForm = ({ team, user }) => {
  ...
  // };
  const handleSubmit = async (formData) => {
    const selectedType = formData.get("type") || type; // Get from formData if not in URL
    ...
    if (isUpdating) {
      ...
      if (!selectedType) {
        // Use selectedType instead of just type
        toast.error("Team type is required");
        return;
      }
        ...
        <div className="flex max-md:flex-col w-full mt-3 items-center justify-between gap-5">
          <div className="w-[550px] max-xl:w-[400px] max-2xl:w-[450px] max-lg:w-full">
            ...
            <div className="relative w-full z-20">
              <select
                name="size"
                className="relative z-20 w-full appearance-none rounded-lg border border-stroke dark:border-dark-3 bg-transparent py-[10px] px-5 text-black outline-none transition focus:border-indigo-500 active:border-primary disabled:cursor-default disabled:bg-gray-200"
              >
                <option value="0" className="dark:bg-dark" default>
                  Select
                </option>
                <option value="8" className="dark:bg-dark">
                  8 Players (6 + 2 reserve )
                </option>
                <option value="10" className="dark:bg-dark">
                  10 Players (8 + 2 reserve){" "}
                </option>
                <option value="13" className="dark:bg-dark">
                  13 Players (11 + 2 reserve)
                </option>
              </select>
            ...
            <div className="w-[550px] max-xl:w-[400px] max-2xl:w-[450px] max-lg:w-full">
              ...
              <div className="relative w-full z-20">
                ...
                >
                  ...
                  <option value="student" className="dark:bg-dark">
                    Students
                  </option>
                  <option value="teacher" className="dark:bg-dark">
                    Teachers
                  </option>
                  <option value="staff" className="dark:bg-dark">
                    Staff
                  </option>
                </select>
                <span className="absolute right-4 top-1/2 z-10 mt-[-2px] h-[10px] w-[10px] -translate-y-1/2 rotate-45 border-r-2 border-b-2 border-body-color"></span>
