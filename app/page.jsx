    ...
    <div className="w-full flex flex-col items-center relative">
      ...
      <section className="w-4/5 h-15 flex flex-row items-center justify-between py-10 max-md:flex-col max-sm:w-full">
        <div className="flex flex-col max-md:items-center max-md:order-2">
          {session?.user ? (
            <>
              <p className="mt-5 text-4xl font-extrabold max-md:text-center max-sm:text-3xl">
                Hello
              </p>
              <p className="text-4xl font-extrabold text-indigo-500 max-md:text-center">
                {session.user.name}
              </p>
              <p className="text-slate-600 font-medium w-4/5 my-4 max-md:text-center max-sm:w-3/4">
                Get started with organizing your team by clicking on the get
                started button. You can get more info about the rules and
                payment procedure by pressing on the learn more button
              </p>
            </>
          ) : (
            <>
              <p className="mt-5 text-5xl font-extrabold max-md:text-center max-sm:text-4xl">
                Welcome to Hostel Premier League
              </p>
              <p className=" text-4xl font-extrabold text-indigo-500 max-md:text-center">
                A Thrilling Cricket Experience
              </p>
              <p className="text-slate-600 font-medium w-1/2 my-4 max-md:text-center max-sm:w-3/4">
                Experience the thrill of the game, witness top teams in action,
                and create lasting memories at this unmissable sporting
                spectacle.
              </p>
            </>
          )}
        ...
        <div className="max-md:flex-col w-4/5 max-w-7xl rounded-lg bg-white border border-slate-300 mb-10 py-6 px-16 flex flex-row items-center justify-between max-sm:py-0 max-md:border-none max-sm:w-full max-sm:px-10 max-sm:my-10">
          <div className="flex flex-col max-md:order-2 max-md:mb-5">
            <div className="text-2xl font-extrabold mb-5 max-md:w-full max-md:text-center">
              <p className="text-emerald-500 max-md:w-full max-md:text-center text-3xl">
                Manage Your Team
              </p>
              <p className="text-slate-700 mt-5 text-lg font-medium">
                Navigate your team's journey seamlessly with our Team Dashboard.
                From player management to match schedules, take control of your
                Edwise Premier League experience.
              </p>
            </div>
            <Link href="/team-details">
              <button
                type="button"
                className="btn_black rounded-md w-fit max-md:w-full"
              >
                View/Fill Team Details
              </button>
            </Link>
          </div>
          <Image
      ...
      <section
        id="info"
        className="w-4/5 max-w-7xl  flex flex-col items-center justify-center py-10 mt-10 max-sm:w-full"
      >
        <div className="flex flex-row items-center border border-slate-400 rounded-sm px-10 max-md:flex-col max-md:border-none">
          ...
          <div className="flex flex-col max-md:items-center max-md:order-2">
            <div className="text-3xl font-extrabold mb-5 max-sm:text-2xl max-md:w-full max-md:text-center max-sm:mt-2">
              ...
              <p className="text-slate-700 mt-5 text-lg font-medium">
                Stay tuned for thrilling matchups! Check out the schedule for
                upcoming Edwise Premier League matches. Plan your game days and
                be part of the excitement.
              </p>
            </div>
            <div className="flex items-center gap-5">
              <p className="bg-red-400 px-5 py-1.5 rounded-md w-fit max-sm:text-center text-white cursor-pointer hover:bg-red-500 transition duration-150">
                Check Timings
              </p>
              <p className="flex px-5 py-1.5 cursor-pointer rounded-md w-fit max-sm:text-center">
                Download Schedule
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-row items-center border border-slate-400 rounded-sm px-10 max-md:flex-col max-md:border-none">
          ...
          <div className="flex flex-col max-md:items-center max-md:order-2">
            <div className="text-3xl font-extrabold mb-5 max-sm:text-2xl max-md:w-full max-md:text-center max-sm:mt-2">
              ...
              <p className="text-slate-700 mt-5 text-lg font-medium">
                We're here to assist you. Whether you have questions about
                registrations, events, or general inquiries, our team is ready
                to help. Reach out to us for prompt assistance.
              </p>
            </div>
            <Link href="/contact-us">
              <p className="bg-blue-400 px-5 py-1.5 rounded-md w-fit max-sm:text-center text-white cursor-pointer hover:bg-blue-500 transition duration-150">
                ContactUs
              </p>
            </Link>
          </div>
      ...
      <section
        ...
        className="w-4/5 max-w-7xl  flex flex-col items-center justify-center py-10 mt-10 max-sm:w-full"
      >
        <p className="text-5xl mb-5 font-extrabold max-md:text-4xl max-sm:text-3xl max-md:text-center">
      ...
      <section
        id="faq"
        className="w-4/5 max-sm:w-full py-24 px-6 max-w-[80rem] mx-auto"
      >
        <div className="max-w-[56rem] mx-auto flex flex-col w-full">
          <p className="text-3xl font-extrabold mb-8 max-md:text-center">
            Frequently Asked Questions
          </p>
          <div className="flex flex-col items-center">
            {frequentQuestions.map((data) => (
              <FAQComponent key={data.question} data={data} />
            ))}
          </div>
        </div>
      </section>
