...
// const Login=()=>(
// 	<div className="h-[calc(100vh-200px)] min-h-[600px] w-full flex items-center justify-center mt-20">
...
// 		      </div>
// 		    </div>
//           <button type="button" className="btn_black w-full rounded-sm" onClick={handleAdminLogin}>Login</button>
...
const Login = () => {
  ...
  const handleAdminLogin = async (e) => {
    ...
    if (res?.error) {
      ...
      router.push("/admin/dashboard"); // Correct way to redirect
    }
  };

  return (
    <div className="h-[calc(100vh-200px)] min-h-[600px] w-full flex items-center justify-center mt-20">
      <section className="relative flex w-[400px] flex-col items-start justify-center py-10 px-5 shadow-md shadow-slate-600">
        <div className="flex flex-row mb-4 items-center justify-start">
          <Image height={32} width={32} alt="Logo" src="/logo.svg" />
          <p className="text-lg font-bold ml-2">Community Premier League</p>
        </div>
        <p className="font-medium text-md mb-5">Admin Login</p>

        {/* Email Input */}
        <div className="w-full mb-6">
          <label className="mb-2 text-base font-medium">Email</label>
          <input
            type="email"
            placeholder="info@yourmail.com"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="w-full bg-transparent rounded-md border border-stroke py-[10px] pr-3 pl-3 text-dark-6 outline-none transition focus:border-primary"
          />
        </div>

        {/* Password Input */}
        <div className="w-full mb-6">
          <label className="mb-2 text-base font-medium">Password</label>
          <input
            type="password"
            placeholder="****"
            value={password}
        ...
        <button
          type="button"
          className="btn_black w-full rounded-sm"
